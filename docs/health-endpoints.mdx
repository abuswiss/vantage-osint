...
The August 2026 rollout grace periods have expired. All eight market coverage checks now classify absent, empty, stale, and partial data using the normal health rules, regardless of whether a market has published before.

Each check retains its `activated` field for diagnosis. The daily publisher writes the durable `seed-activated:consumer-prices:coverage:v1:<market>` marker after publishing coverage with attempted pages; the short-lived manual fallback does not activate a market. Activation does not excuse a failed or missing coverage snapshot.
