...
2026 年 8 月的上线宽限期已结束。八个市场的覆盖率检查现在统一按常规健康规则判定缺失、为空、过期和部分数据，不再因市场尚未发布过数据而降低告警级别。

每项检查仍保留 `activated` 字段供诊断使用。每日发布器发布包含已尝试页面的覆盖率后，会写入持久标记 `seed-activated:consumer-prices:coverage:v1:<market>`；短期手动兜底不会激活市场。激活状态不会豁免缺失或失败的覆盖率快照。
