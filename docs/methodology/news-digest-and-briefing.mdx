...
`full` is the only server digest variant: `listFeedDigest` accepts only the
variant above and normalises any other request, including the retired
`variant=energy`, to `full`. Categories absent from the server digest use the
direct per-feed fallback path instead of a dedicated
`news:digest:v1:{variant}:{lang}` cache key.
...
parsed feeds are cached for 600 seconds; zero-from-zero or failed parses are
...
In the intelligence brief path, feel-good and lifestyle items are excluded
because the brief is meant to be event-driven global intelligence.
